...
export class BillPaymentFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.formId !== 'new') {
      ...
      this.billPaymentService.show(this.formId).pipe(
  ...
  save(continueForm: boolean): void {
    ...
    if (!(this.validationFields.filter(v => v.validation === false).length > 0)) {
      this.billPaymentService.save(this.formId, this.myForm.value).pipe(
        finalize(() => this.loadingFull.active = false),
        catchError((error) => {
  ...
  sumValues(): void {
    if (this.myForm.value.status) {
      ...
      this.myForm.controls['bill_value'].setValue(parseFloat(netTotal.toString()).toFixed(2));
    }
  }
}
